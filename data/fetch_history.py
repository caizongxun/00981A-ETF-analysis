...
特徵設計：全部使用股價類，各期特徵在該期最後一個交易日計算
  - mom_1m / mom_3m / mom_6m: 動量
  - vol_20d / vol_60d: 波動度
  - rs_vs_market: 相對 0050 強弱
...
CACHE_FILE  = DATA_DIR / "period_holdings.json"  # 斷點續跑快取
...
if __name__ == "__main__":
    ...
    if CACHE_FILE.exists():
        ...
        print(f"載入快取: 已有 {len(period_holdings)} 期 -> {sorted(period_holdings.keys())}")
    else:
        period_holdings = {}
    ...
    while cur <= END:
        ...
        if ids:
            ...
            CACHE_FILE.write_text(
                json.dumps({k: list(v) for k, v in period_holdings.items()},
                           ensure_ascii=False), encoding="utf-8"
            )
        else:
            print("無資料（將跳過）")
        cur += relativedelta(months=1)
        time.sleep(2.0)  # 每次查詢多等一點，降低被擋風險
